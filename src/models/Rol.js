...
class Rol {
  ...
  async update(id, nombre) {
    try {
      const [result] = await connection.query(`UPDATE roles SET nombre_rol = ? WHERE id = ?`, [nombre, id]);
  ...
  async delete(rolId) {
    ...
    const [result] = await connection.query("DELETE FROM roles WHERE id = ?", [rolId]);
